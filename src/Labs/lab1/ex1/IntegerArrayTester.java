...
final class IntegerArray {
    ...
    public IntegerArray getSorted() {
        int[]sorted = a.clone();
        return new IntegerArray(Arrays.stream(sorted).sorted().toArray());
    }


    public IntegerArray concat(IntegerArray ia) {
        int[] concat = new int[a.length + ia.length()];
        int j = 0;
        for (int i = 0; i < concat.length; i++) {
            if (i < a.length) {
                concat[i] = a[i];
            } else {
                concat[i] = ia.getElementAt(j++);
            }
        }
        return new IntegerArray(concat);
    }
    ...
    public String toString() {
        return String.format("%s", Arrays.toString(a));
    }
}
